...
/**
 * Maintenance commands for cleaning orphan metadata and other housekeeping
 * tasks.
 * 
 * <p>
 * These commands help maintain the health of the metadata store by identifying
 * and removing metadata entries that are no longer referenced by any snapshot.
 * </p>
 * 
 * <h2>Memory Optimization with Bloom Filter</h2>
 * <p>
 * Uses Guava's BloomFilter for memory-efficient hash lookup (~6 MB for 5M
 * records
 * vs ~680 MB with HashSet). The 1% false positive rate means some orphans may
 * be
 * kept, but no valid metadata will ever be deleted (safe operation).
 * </p>
 */
@ShellComponent
@Configuration
@ComponentScan(basePackages = { "org.lareferencia.core.repository.validation" })
public class MaintenanceCommands {

    private static Logger logger = LogManager.getLogger(MaintenanceCommands.class);
    ...
    @Autowired
    private OAIRecordCatalogRepository oaiRecordRepository;
     ...
     * <h3>Memory Usage</h3>
     * <p>
     * Uses Bloom Filter with 1.2x capacity and 1% false positive rate.
     * For 5M records: ~6 MB vs ~680 MB with HashSet.
     * </p>
     * 
     * <h3>Safety</h3>
     * <p>
     * False positives only mean some orphans are kept (not deleted).
     * Valid metadata is NEVER deleted due to false positives.
     * </p>
     * 
     * @param snapshotId the snapshot ID to clean orphan metadata for
     * @param dryRun     if true, only reports what would be deleted without
     *                   actually deleting
     * @return summary of the cleanup operation
     */
    @ShellMethod(value = "Clean orphan metadata entries not referenced by any record in the snapshot", key = "clean-orphan-metadata")
    public String cleanOrphanMetadata(
            @ShellOption(help = "Snapshot ID to clean orphan metadata for (optional if networkId is provided)", defaultValue = ShellOption.NULL) Long snapshotId,
            @ShellOption(help = "Network ID to get last good known snapshot (optional if snapshotId is provided)", defaultValue = ShellOption.NULL) Long networkId,
        ...
        try {
            ...
            result.append(mode).append("Orphan metadata cleanup completed for snapshot ").append(effectiveSnapshotId)
                    .append("\n");
            ...
            result.append("Active hashes collected: ").append(oaiHashCount.get() + validationHashCount.get())
                    .append("\n");
    ...
    private long countOaiRecords(SnapshotMetadata snapshotMetadata) throws IOException {
        // Use catalog repository's streamAll method
        try (Stream<OAIRecord> stream = oaiRecordRepository.streamAll(snapshotMetadata)) {
            return stream.count();
        }
    }
    ...
    private long countValidationRecords(SnapshotMetadata snapshotMetadata) throws IOException {
        return validationRepository.count(snapshotMetadata.getSnapshotId());
    }
    ...
    /**
     * Collects publishedMetadataHash values from validation records into the Bloom
     * Filter.
     */
    private void collectValidationHashes(SnapshotMetadata snapshotMetadata,
            BloomFilter<CharSequence> activeHashes,
            AtomicLong hashCount) throws IOException {
        try (Stream<ValidationRecord> stream = validationRepository.streamAll(snapshotMetadata.getSnapshotId())) {
            stream.forEach(record -> {
