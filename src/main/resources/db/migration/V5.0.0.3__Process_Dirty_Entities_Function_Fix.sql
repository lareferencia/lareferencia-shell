...
BEGIN
    ...
    -- Delete relation_fieldoccr for relations involving dirty entities (MUST be before deleting relations!)
    RAISE NOTICE 'Deleting relation_fieldoccr for relations involving dirty entities...';
    DELETE FROM relation_fieldoccr rfo
    WHERE EXISTS (
        SELECT 1 FROM aux_entity_map aem
        WHERE aem.entity_id = rfo.from_entity_id OR aem.entity_id = rfo.to_entity_id
    );
